/* $Id: sdl_v.cpp 18809 2010-01-15 16:41:15Z rubidium $ */
...
extern unsigned long VideoAddress[20][1024*768]; // cheating
...
{
  SDL_Window *window = SDL_GetWindowFromID(SDL_GetFocusWindow());
...
static bool CreateMainSurface(uint w, uint h)
{
  ...
  newscreen = SDL_CALL SDL_SetVideoMode(w, h, 32, SDL_SWSURFACE | SDL_HWPALETTE | SDL_RESIZABLE | SDL_NOFRAME);
	...
	fprintf(stderr, "SDL: set mode %ux%ux%d 0x%x\n", newscreen->w, newscreen->h, _screen.pitch, _screen.dst_ptr);
  memset(_screen.dst_ptr, 0, _screen.width*_screen.height*4);
	fprintf(stderr, "SDL: set mode %ux%ux%d 0x%x\n", newscreen->w, newscreen->h, _screen.pitch, _screen.dst_ptr);
...
const char *VideoDriver_SDL::Start(const char * const *parm)
{
  ...
  _cur_resolution.width = 768;
  _cur_resolution.height = 1024;
  ...
  SDL_CALL SDL_EventState(SDL_WINDOWEVENT, SDL_ENABLE);
...
{
  int old_x = 0, old_y = 0, count_xy = 0;
	...
	for (;;) {
		...
		if (0)//keys[SDLK_TAB] && (mod & KMOD_ALT) == 0)
#endif
		{
      ...
      _ctrl_pressed  = SDL_GetMouseState(2, NULL, NULL) & SDL_BUTTON(SDL_BUTTON_LEFT); //!!(mod & KMOD_CTRL);
			_shift_pressed = SDL_GetMouseState(1, NULL, NULL) & SDL_BUTTON(SDL_BUTTON_LEFT); //!!(mod & KMOD_SHIFT);
